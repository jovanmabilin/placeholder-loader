<template>
    <div class="m-0 p-0">
        <div class="placeholder-content">
            <div class="placeholder-content-item mb-3" :style="placeholderItemStyle()"></div>
            <b-row>
                <b-col v-for="indexColumn in column" :key="indexColumn">
                    <template v-if="indexColumn === 1">
                         <div class="placeholder-content-item rounded" :style="placeholderItemStyle(200,100)"></div>
                          <div class="placeholder-content-item" :style="placeholderItemStyle()"></div>
                    </template>
                    <template v-if="indexColumn === 2">
                        <div  class="placeholder-content-item-container" v-for="indexRow in row" :key="indexRow">
                            <div class="placeholder-content-item" :style="placeholderItemStyle()"></div>
                        </div>
                    </template>
                    <template v-if="indexColumn > 2">
                        <div  class="placeholder-content-item-container" v-for="indexRow in row" :key="indexRow">
                            <div class="placeholder-content-item" :style="placeholderItemStyle()"></div>
                        </div>
                    </template>
                </b-col>
            </b-row>
        </div>
    </div>
</template>

<script>
    export default {
        props: {
            row: {
                type: Number,
                default: 6,
            },
            column: {
                type: Number,
                default: 2
            }
        },
        data() {
            return { 
                isLoading: true,
                randomWidth: [20,30,40,50,60],
            }
        },
        methods: {
            placeholderItemStyle(width,height) {
                let random = Math.ceil( Math.random() * ( this.randomWidth.length - 1) );
                let randomWidth = this.randomWidth[random];
                return {
                    width: ( width ) ? `${width}px` : `${randomWidth}%`,
                    height: ( height ) ? `${height}px` : '12px',
                }
            }
        },
    }
</script>

<style lang="scss" scoped>
.placeholder-content {
  width: 100%;
  height: inherit;
  animation: reduce-opacity 2s linear infinite;
}

.placeholder-content .placeholder-content-item  {
  border-radius: 50px;
  margin-bottom: 10px;
  background-color: #dddddd80;
}

.placeholder-content-item-container .placeholder-content-item  {
    display: inline-block;
}

@keyframes reduce-opacity {
      0% { -webkit-filter:    opacity(1);}
      50% { -webkit-filter:   opacity(.1);}
      100% { -webkit-filter:  opacity(1);}
}

.rounded {
    border-radius: 0.25rem !important;
}
</style>